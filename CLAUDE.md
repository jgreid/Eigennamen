...
- `addBot` and `removeBot` run under a per-room `bot-manage:` lock so simultaneous `bot:add` calls can't double-seat the same team+role and a remove can't interleave with an add's seat check
...
- Peer identity is opaque (N1): peers see only the derived 16-hex `playerId` (`player/publicId.ts`); `toPublicPlayer` strips `sessionId`/`lastIP`/`userId` from every peer-facing payload and `toPublicRoom` replaces the room's `hostSessionId` with `hostPlayerId` (R1) — never emit a raw `Room`; adopting a session at the socket handshake additionally requires the per-session `sessionToken` secret (`player/sessionAuth.ts`) delivered only to that client; peer-targeting payloads (`player:kick`, `bot:remove`, `spectator:approveJoin`) carry `playerId`, resolved via `findPlayerByPublicId`
...
- `botController` coalesces mutations that arrive mid-tick (`pending` → re-tick) so a notification landing between a tick's final state read and its exit isn't dropped (a dropped one stalls the bot); it re-verifies the seat (and the game identity) immediately before every apply — pace or no pace — so a removed/kicked bot can't land one last move, and `removeBot` takes the same `bot-manage:` lock as `addBot` (R19)
...
- `joinRoom` for a session that is still a member of another room leaves that room first (R5); `getPlayersInRoom` drops any member whose player hash points at a different room, so a stale member-set entry can never count as a live human or win host succession
...
- **[docs/REVIEW_2026-09.md](docs/REVIEW_2026-09.md)** — fourth pass (September 2026, R1–R75): code findings (host `sessionId` leaked in room broadcasts, unthrottled admin-password check on `/health/metrics*`, ghost membership on cross-room `room:join`, `btoa()` throwing on non-Latin-1 standalone word lists, lock budgets below inner operations, shared-connection WATCH isolation), a full doc-drift inventory (SERVER_SPEC payloads, working-directory contradictions, lock TTLs, clue-number semantics), ledger reconciliation (B5/E4 → FIXED, D5 → PARTIAL, H3 re-opened), a consolidation proposal, and a prioritised state-of-the-art idea list. The first tranche (R1, R2, R5, R6, R7, R9, R11, R12, R16, R19) has shipped with regression tests; the rest is open.
...
| [docs/REVIEW_2026-09.md](docs/REVIEW_2026-09.md)               | Fourth review pass (R1–R75) — security/bug findings, doc-drift inventory, ledger reconciliation, consolidation proposal, and a sourced state-of-the-art idea list; additive to the three plans above                                                                                                                                                                              |
