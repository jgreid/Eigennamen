...
function containerExists(name) {
    ...
    return !r.error && r.status === 0 && String(r.stdout).trim().split('\n').includes(name);
}
...
function startManagedRedis(port) {
    ...
    const r = docker(['run', '-d', '--name', MANAGED_NAME, '--restart', 'unless-stopped', '-p', `${port}:6379`, IMAGE]);
    return !r.error && r.status === 0;
}
...
export async function ensureRedis({ quiet = false } = {}) {
    ...
    if (isLocal && dockerAvailable()) {
        log(`↻ No Redis at ${url} — starting a managed Docker container (${MANAGED_NAME})…`);
        if (startManagedRedis(port) && (await waitReachable(host, port))) {
            log(`✓ Redis ready at ${url} (Docker container "${MANAGED_NAME}").`);
            return url;
        }
        console.warn('⚠ Could not start the managed Redis container — is Docker Desktop running?');
    }

    if (explicit) {
        console.warn(`⚠ Redis at ${raw} is unreachable. Start it (try \`npm run redis:up\`); the server will keep retrying.`);
        return url;
    }

    console.warn('⚠ No reachable Redis and Docker is unavailable — falling back to memory mode.');
    console.warn('  Windows has no redis-server binary, so start Docker Desktop (or install Memurai) and re-run.');
    return undefined;
}
...
export function stopManagedRedis() {
    ...
    console.log(r.status === 0 ? `✓ Stopped "${MANAGED_NAME}".` : `⚠ Could not stop "${MANAGED_NAME}".`);
}
