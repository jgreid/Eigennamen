...
const BOARD = new Set(
  (m[1].match(/'[^']+'/g) || []).map((w) => w.replace(/'/g, "").toUpperCase()),
);
...
function validateEntry(clue, entry) {
  ...
  if (!SAFE_WORD.test(word))
    throw new Error(`${clue}: unsafe word ${JSON.stringify(word)}`);
  if (typeof entry === "string") return word;
  if (entry.weight !== undefined && !inUnit(entry.weight))
    throw new Error(`${clue}/${word}: bad weight`);
  if (entry.collocation !== undefined && !inUnit(entry.collocation))
    throw new Error(`${clue}/${word}: bad collocation`);
  if (entry.kind !== undefined && !EDGE_KINDS.includes(entry.kind))
    throw new Error(`${clue}/${word}: bad kind`);
  return { ...entry, word };
}
...
for (const [clue, words] of Object.entries(CANDIDATES)) {
  if (!SAFE_WORD.test(clue.toUpperCase()))
    throw new Error(`unsafe clue key ${JSON.stringify(clue)}`);
  const kept = [];
  const drop = [];
  const seen = new Set();
...
const fmtEntry = (entry) => {
  ...
  if (entry.collocation !== undefined)
    fields.push(`collocation: ${entry.collocation}`);
  return `{ ${fields.join(", ")} }`;
};
...
try {
  ...
  console.warn(
    "(prettier not run — run `npm run format` in server/ to format the output)",
  );
}
...
console.log(
  `Wrote associations.ts: ${clueCount} clues, ${pairCount} clue->word pairs.`,
);
