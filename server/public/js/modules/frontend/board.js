...
export function initBoardEventDelegation() {
    ...
    if (!board || board.hasAttribute('data-delegated'))
        return;
    ...
    board.addEventListener('click', (e) => {
        ...
        if (!isNaN(index) && index >= 0 && cardClickHandler)
...
export function renderBoard() {
    ...
    try {
        ...
        state.gameState.words.forEach((word, index) => {
            ...
            card.setAttribute('aria-label', buildCardAriaLabel(word, isRevealed ?? false, state.gameState.types[index] ?? '', row, col));
            if (isRevealed) {
    ...
    try {
        ...
        for (let index = 0; index < cards.length; index++) {
            ...
            if (isRevealed) {
                ...
                card.classList.remove('revealed', 'red', 'blue', 'neutral', 'assassin', 'just-revealed', 'success-reveal');
            }
