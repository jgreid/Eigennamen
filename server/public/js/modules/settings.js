...
export function openSettings() {
    ...
    if (redNameInput)
        redNameInput.value = state.teamNames.red;
    if (blueNameInput)
        blueNameInput.value = state.teamNames.blue;
...
export function saveSettings() {
    ...
    if (selectedMode === 'custom' && customWords.length < BOARD_SIZE) {
        ...
        if (wordError)
            wordError.classList.add('visible');
        if (textarea) {
            textarea.classList.add('invalid');
            textarea.focus();
        }
    ...
    if (wordError)
        wordError.classList.remove('visible');
    if (textarea)
        textarea.classList.remove('invalid');
    updateURL();
    updateScoreboard();
...
export function resetWords() {
    ...
    const combinedRadio = document.getElementById('wordlist-mode-combined');
    if (combinedRadio) {
