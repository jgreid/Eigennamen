/**
 * Player Socket Event Handlers - REFACTORED VERSION
 *
 * This demonstrates the architectural improvement using PlayerContext.
 * Compare this to the original playerHandlers.js to see the reduction in boilerplate.
 */
...
const { canChangeTeamOrRole, syncSocketRooms } = require('../playerContext');
...
module.exports = function playerHandlers(io, socket) {
     ...
     * Set player's team
     *
     * BEFORE (original): 50+ lines with repetitive validation
     * AFTER (refactored): ~30 lines of actual business logic
     */
    socket.on('player:setTeam', createRoomHandler(socket, 'player:setTeam', playerTeamSchema,
        async (ctx, validated) => {
            // ctx contains: sessionId, roomCode, player, game, isHost, team, role
            // All pre-validated - we know player exists and is in the room

            // Business logic: Check if team change is allowed during game
            ...
            // Determine if we need empty-team validation
            const shouldCheckEmpty = ctx.game && !ctx.game.gameOver &&
                ctx.player.team && ctx.player.team !== validated.team;

            // Perform the team change
            const previousPlayer = { ...ctx.player };
            ...
            // Sync socket room membership (spectator room management)
            syncSocketRooms(socket, player, previousPlayer);
            ...
            return { player }; // Return for socket room sync
        }
    ));
    ...
    socket.on('player:setRole', createRoomHandler(socket, 'player:setRole', playerRoleSchema,
        async (ctx, validated) => {
            // Check if role change is allowed during game
            const canChange = canChangeTeamOrRole(ctx);
            if (!canChange.allowed) {
                throw {
