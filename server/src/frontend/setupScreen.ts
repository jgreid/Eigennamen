...
export function showSetupScreen(): void {
    ...
    const joinNick = document.getElementById('setup-join-nickname') as HTMLInputElement | null;
    const hostNick = document.getElementById('setup-host-nickname') as HTMLInputElement | null;
    if (joinNick) joinNick.value = storedNickname;
    if (hostNick) hostNick.value = storedNickname;
}

/** Hide the setup screen and show the app layout. */
export function hideSetupScreen(): void {
    const setupScreen = document.getElementById('setup-screen');
    const appLayout = document.getElementById('app-layout');
...
function showJoinForm(): void {
    ...
    const nickInput = document.getElementById('setup-join-nickname') as HTMLInputElement | null;
    if (nickInput) setTimeout(() => nickInput.focus(), 50);
}

/** Show the host form and hide the board grid. */
function showHostForm(): void {
    const board = document.getElementById('setup-board');
    const joinForm = document.getElementById('setup-join-form');
    const hostForm = document.getElementById('setup-host-form');
    if (board) board.hidden = true;
    ...
    // Focus nickname input
    const nickInput = document.getElementById('setup-host-nickname') as HTMLInputElement | null;
    if (nickInput) setTimeout(() => nickInput.focus(), 50);
}
...
function setStatus(elementId: string, message: string, type: string): void {
...
async function handleJoinSubmit(): Promise<void> {
    ...
    const nickEl = document.getElementById('setup-join-nickname') as HTMLInputElement | null;
    const roomEl = document.getElementById('setup-join-room-id') as HTMLInputElement | null;
    const btn = document.getElementById('setup-join-btn') as HTMLButtonElement | null;
...
async function handleHostSubmit(): Promise<void> {
    clearAllErrors();

    const nickEl = document.getElementById('setup-host-nickname') as HTMLInputElement | null;
    const roomEl = document.getElementById('setup-host-room-id') as HTMLInputElement | null;
    const redNameEl = document.getElementById('setup-red-name') as HTMLInputElement | null;
    const blueNameEl = document.getElementById('setup-blue-name') as HTMLInputElement | null;
    const gameModeEl = document.querySelector('input[name="setup-gameMode"]:checked') as HTMLInputElement | null;
    const timerToggleEl = document.getElementById('setup-turn-timer-toggle') as HTMLInputElement | null;
    const timerRangeEl = document.getElementById('setup-turn-timer-range') as HTMLInputElement | null;
    const btn = document.getElementById('setup-host-btn') as HTMLButtonElement | null;
...
export function initSetupScreen(): void {
    // Timer toggle
    const timerToggle = document.getElementById('setup-turn-timer-toggle') as HTMLInputElement | null;
    ...
    const timerRange = document.getElementById('setup-turn-timer-range') as HTMLInputElement | null;
...
export function handleSetupAction(action: string): void {
    switch (action) {
        case 'setup-host':
            showHostForm();
            break;
