...
/** Minimal interface the room actions need from the main adapter. */
export interface RoomActionHost {
...
function generateRequestId(host: RoomActionHost): string {
    host._nextRequestId = (host._nextRequestId + 1) % Number.MAX_SAFE_INTEGER;
    return 'req_' + host._nextRequestId;
}

function getSocket(host: RoomActionHost): SocketClientInstance | null {
    if (!host.socket) {
...
export function createRoom(
    host: RoomActionHost,
    options: CreateRoomOptions = { roomId: '' }
 ...
 */
export function joinRoom(host: RoomActionHost, roomId: string, nickname: string): Promise<JoinCreateResult> {
 ...
 * @param host - The adapter object
 */
export function requestResync(host: RoomActionHost): Promise<ClientEventMap['roomResynced']> {
    return new Promise((resolve, reject) => {
        if (!host.roomCode) {
