...
describe('retryAsync', () => {
    ...
    it('retries on failure and returns result on success', async () => {
        ...
        expect(mockLogger.warn).toHaveBeenCalledWith(
            expect.stringContaining('test-op failed (attempt 1/4)')
        );
    });
    ...
    it('throws after exhausting all retries', async () => {
        ...
        await expect(
            retryAsync(fn, { maxRetries: 2, baseDelayMs: 1, operationName: 'doomed' })
        ).rejects.toThrow('persistent-fail');

        // initial + 2 retries = 3
        expect(fn).toHaveBeenCalledTimes(3);
        expect(mockLogger.error).toHaveBeenCalledWith(
            expect.stringContaining('doomed failed after 3 attempts')
        );
    });
    ...
    it('uses default options when none provided', async () => {
        ...
        expect(result).toBe('ok');
        expect(mockLogger.warn).toHaveBeenCalledWith(
            expect.stringContaining('operation failed (attempt 1/4)')
        );
    });
    ...
    it('handles zero maxRetries (no retries)', async () => {
        ...
        await expect(
            retryAsync(fn, { maxRetries: 0, baseDelayMs: 1 })
