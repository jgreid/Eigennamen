 ...
 */
export async function getSpectatorCount(roomCode: string, existingPlayers?: Player[]): Promise<number> {
 ...
 */
export async function getRoomStats(roomCode: string, existingPlayers?: Player[]): Promise<RoomStats> {
    const players = existingPlayers || (await getPlayersInRoom(roomCode));
