...
**Status of this document:** most items have shipped — each fixed item's header is marked **FIXED** with a resolution note. Still open: B8, B15, C7, C9, D5, D7, E1, plus the partially-done B5, E2, and E4. Treat each item's status marker as the source of truth, and update it in the same PR that closes the item, mirroring HARDENING_PLAN.md's convention.
...
**Deployment context used for severity calls (historical — review-time baseline):** at review time, production was a single Fly.io machine running memory-mode Redis (`fly.toml` then had `REDIS_URL = "memory"`, deploy `strategy = "immediate"`, a 512 MB VM, and embeddings opt-in and disabled), and severities were rated against that reality. Since then production has moved to external Redis via a required `REDIS_URL` secret, a 2 GB VM, and an enabled wide-embeddings bake (still a single machine, still `strategy = "immediate"`) — so the "memory-mode wipes state on deploy" severity weighting no longer applies, while the single-instance caveats (HARDENING_PLAN Phase 2) still do. Several items note explicitly how their severity changes under multi-instance scaling.
