...
| **N19** | `game:typing` indicator (server accepts + rebroadcasts, no client emitter/listener) | **Delete** — it escaped this Phase-F pass; the frontend never emits it or registers a listener, so remove the handler + `SOCKET_EVENTS.GAME_TYPING` + rate-limit entry + doc row rather than half-wire a typing indicator nobody built | **Shipped (delete path)** — server handler, socket-event constant, `game:typing` rate-limit entry, and the CLAUDE.md event-table row all removed. Re-add both ends together if a typing indicator is ever built. |
