...
### N1 — A player's session can be adopted by any room peer; `sessionId` is the only credential and it is broadcast to everyone — **FIXED**

**Resolution:** Both mitigations shipped in the dedicated N1 PR. (1) Peers never see a `sessionId`: `toPublicPlayer` strips it and adds an opaque derived `playerId` (`services/player/publicId.ts`, sha256 prefix — deterministic, no storage/Lua changes); every peer-facing emit (room/player/game/chat/spectator/bot/ready/typing/hostChanged) and every client→server target payload (`player:kick`, `bot:remove`, `spectator:approveJoin`) was rekeyed, with server-side resolution via `findPlayerByPublicId`. A player's own `sessionId` survives only in direct-to-self payloads (`toSelfPlayer`). (2) Session adoption is gated on a server-issued per-session secret (`services/player/sessionAuth.ts`): minted at room create/join, delivered privately in `room:created`/`room:joined`/`room:reconnected` as `sessionToken`, persisted by the client next to its sessionId (same sessionStorage — no reconnect Catch-22), sent in the handshake `auth`, and required by `resolveSessionId` for BOTH the connected-continuity and disconnected branches (a session with no stored secret — pre-deploy legacy — grandfathers through the old IP checks, bounded by the player-record TTL). En passant this also closed the one `ROOM_PLAYER_LEFT` emit (`bot:remove`) that still bypassed N2's PII projection. Regression coverage: `playerPublicId.test.ts`, `sessionAuth.test.ts`, `sessionAdoption.test.ts` (adopting a peer's sessionId without the secret must fail; no peer payload carries sessionId).
