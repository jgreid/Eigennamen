...
**Status of this document:** Phase 0 (all 5 items) has shipped, with regression tests, and the full suite (167 suites / 4181 tests; baseline at the time of the review; current is 179 suites / 4494 tests), lint, and typecheck all pass. Phase 0 and Phase 1 have shipped; Phase 2 is planning only. The documentation fixes in [Phase 3 / Doc hygiene](#doc-hygiene-done-this-pass) landed earlier. Treat each item's status marker as the source of truth for what's actually shipped — update it in the same PR that closes the item.
